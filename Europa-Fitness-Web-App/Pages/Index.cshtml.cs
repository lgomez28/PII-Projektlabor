...
int indexsameday(List<WorkoutModel> list, string uebung)
{
    for(int i=0;i<list.Count();i++){
        if(list[i].Uebung==uebung){
            return i;
        }
    }
    return 99999;
...
public string[] get_timestamp(){
      ...
      /*  public string[] get_timestamp(){
            ...
            string [] timestamplist = new string [WorkoutArray.Length];

            for(int i=0;i<WorkoutArray.Length;i++){   
                try{
                    timestamplist[i]=(WorkoutArray[i].ID);
                    int position = timestamplist[i].IndexOf(" ");
                    timestamplist[i]=timestamplist[i].Substring(0,position);
                }
                catch{
